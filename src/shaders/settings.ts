...
const skybox = /*glsl*/ `
    ...
    const vec3 STAR_COLOR_0 = vec3(1.0, 0.95, 0.9);
    const vec3 STAR_COLOR_1 = vec3(1.0, 0.9, 0.9);
    const vec3 STAR_COLOR_2 = vec3(0.9, 1.0, 1.0);
    const vec3 STAR_COLOR_3 = vec3(0.9, 0.95, 1.0);
    const vec3 STAR_COLOR_4 = vec3(1.0, 0.9, 1.0);
    const vec3 STAR_COLOR_5 = vec3(1.0, 1.0, 1.0);

    vec3 getStarColor(int idx) {
        if (idx == 0) return STAR_COLOR_0;
        if (idx == 1) return STAR_COLOR_1;
        if (idx == 2) return STAR_COLOR_2;
        if (idx == 3) return STAR_COLOR_3;
        if (idx == 4) return STAR_COLOR_4;
        return STAR_COLOR_5;
    }
    ...
    vec3 sampleSkybox(vec3 dir) {
        ...
        sky = mix(sky, max(getStarColor(int(gridValue.w * 6.0)), vec3(moon)), stars);
...
export function initShaderChunks() {
  if (initialized) return
  initialized = true

  const chunks = THREE.ShaderChunk as Record<string, string>
  chunks['global'] = global
  chunks['skybox'] = skybox
  chunks['ocean'] = ocean
}
